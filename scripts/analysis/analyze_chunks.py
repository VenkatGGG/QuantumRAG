...
def analyze_corpus_from_articles() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    if overlap_tokens:
        ...
        if zero_overlaps:
            ...
            for o in zero_overlaps[:5]:  # nosec: analysis output, not sensitive
                print(f"    Article: {o['article']}")  # nosec: article title for analysis
    ...
    # Show smallest chunks (limited output for analysis)
    print(f"\n{'='*60}")
    ...
    for c in sorted(small_chunks, key=lambda x: x['tokens'])[:10]:
        print(f"  {c['tokens']} tokens - article_id={hash(c['article']) % 10000:04d}")  # nosec: hashed identifier only
